// weekly_quiz.js - Complete Version with Database Session Management
...
async function clearSessionFromStorage() {
    try {
        ...
        localStorage.removeItem(`weekly_quizzes_${currentUsername}`);
...
async function loadQuizzes() {
    ...
    try {
        ...
        localStorage.setItem(`weekly_quizzes_${currentUsername}`, JSON.stringify(data));

    } catch (err) {
        console.error("Failed to load quizzes:", err);
        
        // Try to load from cache
        const cached = localStorage.getItem(`weekly_quizzes_${currentUsername}`);
        if (cached) {
            try {
...
async function downloadAllCharts() {
    ...
    html2canvas(temp, { 
        ...
        link.download = `weekly_quizzes_${currentUsername}_${date}.png`;
...
function exportToCSV() {
    ...
    const quizzes = JSON.parse(localStorage.getItem(`weekly_quizzes_${currentUsername}`) || '[]');
    if (quizzes.length === 0) {
    ...
    link.setAttribute('download', `weekly_quizzes_${currentUsername}_${new Date().toISOString().split('T')[0]}.csv`);
